...
The exact policy checkpoints used in the article will be deposited in an immutable archive before submission. The archive will include the model manifest and SHA-256 hashes. Its DOI will be added here after deposition. Evaluation scripts expect the checkpoints under `results/raw/` using the filenames listed in `MODEL_MANIFEST.csv`.
