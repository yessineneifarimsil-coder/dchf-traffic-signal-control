...
QMIX_TRAINED_MODEL = "results/raw/qmix_corridor_model_v2_turning.pth"

OUTPUT_RAW = "results/raw/multiseed_scenario_b_raw.csv"
OUTPUT_SUMMARY = "results/tables/multiseed_scenario_b_summary.csv"
...
def collect_qmix_step_row(env, controller, seed, decision, actions, reward, done):
    return {
        "controller": controller,
        "seed": seed,
        "decision": decision,
...
def run_qmix_yellow_with_logging(env, controller, seed, selected_phases, rows, decision, actions):
    yellow_needed = False
...
def run_qmix_green_with_logging(env, controller, seed, selected_phases, rows, decision, actions):
    for tl_id in env.tl_ids:
...
def run_qmix_model(seed, model_path, controller_name):
    env = TwoIntersectionEnv(
        sumo_binary=SUMO_BINARY,
        sumo_config=SUMO_CONFIG,
        simulation_steps=SIMULATION_STEPS,
    ...
    return rows, summarize_rows(controller_name, seed, rows)
...
def main():
    ...
    experiments = [
        ("Best Offset Fixed-Time 45s", "offset", None),
        ("QMIX V2 transferred", "qmix", QMIX_TRANSFERRED_MODEL),
        ("QMIX V2 trained on Scenario B", "qmix", QMIX_TRAINED_MODEL),
    ]

    for seed in SEEDS:
        print(f"\n===== Scenario B seed {seed} =====")

        for controller_name, controller_type, model_path in experiments:
            print(f"Running {controller_name}...")

            if controller_type == "offset":
                rows, summary = run_best_offset(seed)
            else:
                rows, summary = run_qmix_model(seed, model_path, controller_name)
    ...
    print("\n=== Scenario B per-seed summary ===")
    print(summary_seed_df.to_string(index=False))

    print("\n=== Scenario B multi-seed summary ===")
